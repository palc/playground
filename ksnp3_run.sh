...
#rm fasta_list in_list my_fasta_master.fasta
#grep -v "RAxML" log.txt | grep -v "scratch" | grep -v "fsplit" > log.temp; mv log.temp log.txt
...
echo "" > mytempfile
echo "scratch location: `pwd`" >> mytempfile
echo "network location: /bioinfo11/TStuber/Results/temp/${dir_name}" >> mytempfile
echo "" >> mytempfile
